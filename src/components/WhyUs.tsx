...
const WhyUs = () => {
  ...
  const onSlideChange = useCallback(
    (index: number) => {
      ...
      setDirection(1); // Default direction for tab changes
  ...
  useEffect(() => {
    ...
    const tabInterval = setInterval(() => {
      setActiveIndex((prevActiveIndex) => {
        ...
        imagePositionsRef.current[prevActiveIndex] = currentImageIndex;
        const nextIndex = (prevActiveIndex + 1) % slides.length;

        // Switch to new tab's stored image immediately
        setDirection(1); // Tab changes always go forward
  ...
  return (
    ...
    >
      ...
      <div className="xl:min-h-80 xl:max-h-80 md:min-h-72 md:max-h-72 flex flex-col md:flex-row rounded-xl overflow-hidden md:items-center bg-gray-900">
        <motion.div
          ...
          animate="visible"
          exit="exit"
          className="text-white md:w-1/2 p-6 space-y-4 flex flex-col justify-center md:h-full"
        >
        ...
        <div className="md:w-1/2 flex items-center justify-center relative h-full">
          <button
            ...
            className="absolute left-2 top-1/2 transform -translate-y-1/2 z-20 text-white text-[4rem] px-2 py-1 hover:bg-black/20 rounded transition-colors"
          >
            ‹
          </button>
          ...
          <div className="w-full relative flex items-center justify-center">
            ...
            {/* Main image with right-to-left animation */}
            <motion.img
          ...
          <button
            ...
            className="absolute right-2 top-1/2 transform -translate-y-1/2 z-20 text-white text-[4rem] px-2 py-1 hover:bg-black/20 rounded transition-colors"
          >
            ›
          </button>
        </div>
